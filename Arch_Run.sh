...
echo '> Checking: OS'
if grep -Fxq 'ID=arch' /etc/os-release || grep -Fxq 'ID=manjaro' /etc/os-release || grep -Fxq 'ID=endeavouros' /etc/os-release; then
    echo "System compatible with script"
    ...
    echo "This script is for Arch Linux, Manjaro, or EndeavourOS only. You can still install the dotfiles manually!"
fi